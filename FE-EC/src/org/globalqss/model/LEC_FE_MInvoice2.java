...
import org.globalqss.util.SRIXMLSignature;
...
@Deprecated
public class LEC_FE_MInvoice2 extends MInvoice
{
	/**
	 * 
	 */
	...
	public LEC_FE_MInvoice2(Properties ctx, int C_Invoice_ID, String trxName) {
	...
	public String lecfeinv_SriExportInvoiceXML100 ()
	{
		...
		if (signature.IsUseContingency) {
			signature.setDeliveredType(SRIXMLSignature.emisionContingencia);
			signature.setCodeAccessType(SRIXMLSignature.claveAccesoContingencia);
		}
		...
		if (get_ValueAsBoolean("IsDiscountPrinted")==true) {
		...
		if (signature.getCodeAccessType().equals(SRIXMLSignature.claveAccesoAutomatica))
		...
		// New Authorization
		X_SRI_Authorization a = new X_SRI_Authorization (getCtx(), 0, get_TrxName());
		...
		file_name = signature.getFolderRaiz() + File.separator + SRIXMLSignature.folderComprobantesGenerados + File.separator + xmlFileName;	
		...
		sql = new StringBuffer(
	            ...
	            + "JOIN C_Invoice_LineTax_VT ilt ON ilt.C_Invoice_ID = il.C_Invoice_ID AND ilt.C_InvoiceLine_ID = il.C_InvoiceLine_ID "
		...
		*/
		...
		if (m_sumabaseimponible.compareTo(m_totalbaseimponible) != 0
			&& m_totalbaseimponible.subtract(m_sumabaseimponible).abs().compareTo(SRIXMLSignature.HALF) > 1) {
		...
		if (m_sumavalorimpuesto.compareTo(m_totalvalorimpuesto) != 0
			&& m_totalvalorimpuesto.subtract(m_sumavalorimpuesto).abs().compareTo(SRIXMLSignature.HALF) > 1) {
...
//		if (LEC_FE_Utils.breakDialog("Firmando Xml")) return "Cancelado...";	// TODO Temp
		...
		signature.setOutput_Directory(signature.getFolderRaiz() + File.separator + SRIXMLSignature.folderComprobantesFirmados);
        signature.execute();
        
        file_name = signature.getFilename(signature, SRIXMLSignature.folderComprobantesFirmados);
...
//	        if (LEC_FE_Utils.breakDialog("Enviando Comprobante al SRI")) return "Cancelado...";	// TODO Temp
		    ...
		    file_name = signature.getFilename(signature, SRIXMLSignature.folderComprobantesAutorizados);
	    ...
	    if (MSysConfig.getBooleanValue("QSSLEC_FE_EnvioXmlAutorizadoBPEmail", false, getAD_Client_ID()))
		{
	    	try {
				...
				if (attachment.exists() || attachment.isFile() || attachment.canRead()) {
					...
					mText.setPO(this);
					String subject = "SRI " + (signature.isOnTesting ? SRIXMLSignature.nombreCertificacion : SRIXMLSignature.nombreProduccion) + " " + bpe.getValue() + " : " + f.get_ValueAsString("XmlPrintLabel") + " " + getDocumentNo();
					String text =
							" Emisor               : " + bpe.getName() +
					...
					int countMail = LEC_FE_Utils.notifyUsers(getCtx(), mText, getAD_User_ID(), subject, text, attchs, get_TrxName(),0);
					if (countMail == 0)
