...
public class LEC_InvoiceGenerate extends SvrProcess {
		...
		BigDecimal qtyInvoiced, BigDecimal qtyEntered)
	{
		if (m_invoice == null)
		{
			...
			if (order.get_Value("SRI_ComercioExterior")!=null)
				m_invoice.set_ValueOfColumn("SRI_ComercioExterior",order.get_Value("SRI_ComercioExterior").toString() );
			if (order.get_Value("et_incoterms")!=null)
				m_invoice.set_ValueOfColumn("et_incoterms",order.get_Value("et_incoterms").toString() );		
			//
		}
		//
